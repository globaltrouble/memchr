use crate::arch::all::memchr;

mod default_rank;

// BREADCRUMBS: Time to do memchr stuff? Or should I switch to the memmem
// sub-module and gut it now that all of its components are now in 'arch'?
// Also, don't forget about Shift-Or. Let's do memchr.
...
#[derive(Clone, Debug)]
pub struct Finder {
    // TODO: Replace this with a memchr searcher that isn't tied to the
    // fallback.
    memchr: memchr::One,
    pair: Pair,
    byte2: u8,
...
impl Finder {
    ...
    pub fn with_pair(needle: &[u8], pair: Pair) -> Option<Finder> {
        ...
        let memchr = memchr::One::new(byte1);
        // Currently this can never fail so we could just return a Finder,
        // but it's conceivable this could change.
        Some(Finder { memchr, pair, byte2 })
    }

    /// Run this finder on the given haystack as a prefilter.
    ///
    /// If a candidate match is found, then an offset where the needle would
    /// begin in the haystack (if it's there) is returned.
    #[inline]
    pub fn find_prefilter(&self, haystack: &[u8]) -> Option<usize> {
        ...
        loop {
            ...
            // needle.
            i += self.memchr.find(&haystack[i..])?;

            // If we can't align our first byte match with the haystack, then a
            // match is impossible.
            let aligned1 = match i.checked_sub(index1) {
                None => continue,
...
mod tests {
    ...
    fn forward_packedpair() {
        fn find(
            ...
            Some(Finder::new(needle)?.find_prefilter(haystack))
        }
